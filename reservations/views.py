...
def register_reservation(request):
    ...
    try:
        ...
        if not all([usuario_id, sucursal_id, consola_id, fecha_str, hora_str, num_personas, telefono]):
        ...
        sucursal = Sucursal.objects.get(id=sucursal_id)
        consola = Consola.objects.get(id=consola_id) if consola_id else None
        usuario = User.objects.get(id=usuario_id)

        # Llamar a la función para crear la reserva
        reserva_creada = crear_reserva(usuario, sucursal_id, consola_id, fecha, hora, num_personas, comentarios, telefono)
